...
const FlowDiagram = () => {
  ...
  const onDeploy = useCallback(
    ...
    [projectId, nodes, checkRequiredConnections, setNodes]
  );
  ...
  const updateNodeData = useCallback(
    (nodeId, newData) => {
      setNodes((nds) =>
        nds.map((node) => {
          if (node.id === nodeId) {
            // If the node is being deployed, fetch the latest statistics
            if (newData.deploy_status === 'deployed') {
              apiService
                .getNodeStatistics(projectId, nodeId)
                .then((statistics) => {
                  setNodes((currentNodes) =>
                    currentNodes.map((n) =>
                      n.id === nodeId
                        ? { ...n, data: { ...n.data, statistics } }
                        : n
                    )
                  );
                });
        ...
        <div style={{ flexGrow: 1 }} ref={reactFlowWrapper}>
          ...
          >
            <NodeOperations
              ...
              setEdges={setEdges}
              projectId={projectId}
              setMissingConnections={setMissingConnections}
              setMissingConnectionsModalOpen={setMissingConnectionsModalOpen}
            />
            <EdgeOperations
              nodes={nodes}
              edges={edges}
