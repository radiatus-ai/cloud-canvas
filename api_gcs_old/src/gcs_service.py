...
class GCSService:
    ...
    def append_to_json_file(self, file_name: str, new_item: Any) -> Optional[str]:
        ...
        new_item.update(
            {"id": new_id, "created_at": current_time, "updated_at": current_time}
        )
    ...
    def update_item_in_json_file(
        self, file_name: str, item_id: str, updated_item: Any
    ) -> bool:
        current_content = self.read_json_file(file_name) or []
        for i, item in enumerate(current_content):
            if item.get("id") == item_id:
                updated_item["id"] = item_id  # Ensure ID doesn't change
                updated_item["created_at"] = item.get(
                    "created_at"
