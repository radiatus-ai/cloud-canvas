...
def get_connected_input_data(project_id: str, package_id: str) -> Dict[str, Any]:
    ...
    related_connections = [
        conn for conn in connections_data if conn["target"] == package_id
    ]
    ...
    for conn in related_connections:
        ...
        source_package = next(
            (p for p in packages_data if p["id"] == source_package_id), None
        )
        ...
        if source_package and "output_data" in source_package:
            ...
            if source_handle in source_package["output_data"]:
                connected_input_data[target_handle] = source_package["output_data"][
                    source_handle
                ]
...
def delete_package_entry(project_id: str, package_id: str):
    ...
    if package.get("deploy_status") != DeployStatus.UNDEPLOYED:
        raise HTTPException(
            status_code=400,
            detail="Cannot delete deployed package. Tear down infrastructure first.",
        )
