...
def deploy_package(
    ...
    package_id: str,
    package: Dict,
    connected_input_data: Dict[str, Any],
...
"""
    ...
    for command in commands:
        ...
        output = run_command(
            command, env_vars={"GITHUB_TOKEN": os.environ.get("GITHUB_TOKEN")}
        )
        command_outputs.append(output)
        logger.info(
            f"Command output: {output[:200]}..."
        )  # Log first 200 characters of output
    ...
    output_data = {
        k: processed_output[k]
        for k in package.get("outputs", {}).get("properties", {})
    ...
    logger.info(
        f"Deployment completed successfully for package {package_id} in project {project_id}"
    )
...
def destroy_package(
    ...
    logger.info(
        f"Starting destruction for package {package_id} in project {project_id}"
    )
...
"""
    ...
    logger.info(
        f"Destruction completed successfully for package {package_id} in project {project_id}"
    )
