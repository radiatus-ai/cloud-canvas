...
async def create_project_package(
    ...
    original_package = next((p for p in packages if p['id'] == project_package.package_id), None)
    ...
    new_package = {
        **original_package,
        ...
        'inputs': {**original_package.get('inputs', {'properties': {}}), **project_package.inputs},
        'outputs': {**original_package.get('outputs', {'properties': {}}), **project_package.outputs},
        'parameters': {**original_package.get('parameters', {'properties': {}}), **project_package.parameters},
    }
...
async def update_project_package(
    ...
    file_name = get_project_packages_file_name(project_id)
    if gcs_service.update_item_in_json_file(file_name, package_id, project_package.dict()):
        packages = gcs_service.read_json_file(file_name)
...
async def deploy_project_package(
    ...
    try:
        ...
        update_package_status(project_id, package_id, DeployStatus.DEPLOYED, output_data)
...
async def delete_project_package(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to delete project package: {str(e)}")
